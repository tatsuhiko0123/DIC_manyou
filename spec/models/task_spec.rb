...
describe 'タスクモデル機能', type: :model do
  describe 'バリデーションのテスト' do
    context 'タスクのタイトルが空の場合' do
      it 'バリデーションにひっかる' do
        task = Task.new(title: '', content: '失敗テスト', expired_at: '002021-05-18', status: 1)
    ...
    context 'タスクの詳細が空の場合' do
      it 'バリデーションにひっかかる' do
        # binding.irb
        task = Task.new(title: 'content', content: '', expired_at: '002021-05-18', status: 1)
        expect(task).not_to be_valid
      end
    end
    context 'タスクのタイトルと詳細に内容が記載されている場合' do
      it 'バリデーションが通る' do
        task = Task.new(title: '成功テスト', content: '成功テスト', expired_at: '002021-05-18', status: 1)
  ...
  describe '検索機能' do
    ...
    let!(:task2) { FactoryBot.create(:task, title: 'name', content: 'task2', expired_at: '002021-05-20', status: 2) }
